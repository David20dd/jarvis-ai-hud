...
def _process_telegram_media(session_id: str, event: Dict[str, Any]) -> str:
    ...
    if kind == "audio":
        ...
        reply = _resolve_channel_text("telegram", session_id, transcript)
        return f"Transcripción: “{transcript[:700]}”\n\n{reply}"

    if kind == "image":
        prompt = (
            "Analiza cuidadosamente esta imagen para ayudar al usuario de JARVIS. "
    ...
    if kind == "document":
        file_name = str(event.get("file_name") or Path(str(downloaded.get("file_path") or "documento")).name)
        ...
        prompt = (
            "Analiza el documento enviado por el usuario. Resume los puntos importantes, responde a su comentario "
            "y señala limitaciones sin inventar contenido.\n\n"
            f"Comentario: {caption or 'Resume y explica este documento.'}\n\n"
        ...
        result = resilient_resolve(
            ...
            mode="auto",
            intent_info={"intent": "documents", "confidence": 1.0},
        )
        reply = str(result.get("reply") or "").strip()
        if not reply:
            reply = f"Guardé {file_name} en la Biblioteca con {stored['characters']} caracteres extraídos."
        guardar_mensaje_db(session_id, "user", f"[Documento de Telegram] {file_name}: {caption}".strip())
        guardar_mensaje_db(session_id, "assistant", reply)
        return f"Documento guardado: {file_name}.\n\n{reply}"

    if kind == "video":
        if caption:
            return _resolve_channel_text("telegram", session_id, caption)
        return "Recibí el video. Añade un comentario para indicarme qué necesitas analizar."
    return "Recibí el archivo, pero su formato no es compatible todavía."
