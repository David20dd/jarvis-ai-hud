...
def test_http_health_and_headers():
    with TestClient(main.app) as client:
        ...
        assert live.json()["version"] == "101.0.0"
        assert live.headers["x-jarvis-version"] == "101.0.0"
...
def test_provider_gateway_endpoints_and_route_preview():
    with TestClient(main.app) as client:
        ...
        payload = status.json()
        assert payload["version"] == "101.0.0"
...
def test_v76_owner_password_recovery_revokes_old_sessions():
    db_file = str(Path(tempfile.mkdtemp()) / "owner-recovery.db")
    store = IdentityStore(db_file, session_days=7)
    store.init_schema()
    store.register("owner@example.com", "SecurePassword123", "Owner", role="admin")
    previous = store.login("owner@example.com", "SecurePassword123")
    recovered = store.reset_owner_password("owner@example.com", "NewSecurePassword456")
    assert recovered["role"] == "admin"
    assert store.authenticate(previous["token"]) is None
    with pytest.raises(PermissionError):
        store.login("owner@example.com", "SecurePassword123")
    current = store.login("owner@example.com", "NewSecurePassword456")
    assert store.authenticate(current["token"])["email"] == "owner@example.com"
...
def test_v46_operations_and_channel_status_endpoints():
    with TestClient(main.app) as client:
        ...
        assert operations.json()["version"] == "101.0.0"
...
def test_v47_frontend_is_clean_connected_and_boot_safe():
    ...
    js = Path("static/app.js").read_text(encoding="utf-8")
    css = Path("static/styles.css").read_text(encoding="utf-8")
    assert "¿En qué puedo" in html and "ayudarte hoy?" in html
    ...
    assert "error?.status === 401" in js
    assert "jarvis-reliable-intelligence-v101-1" in Path("service-worker.js").read_text(encoding="utf-8")
...
def test_v56_clean_experience_contract():
    html = Path("index.html").read_text(encoding="utf-8")
    js = Path("static/app.js").read_text(encoding="utf-8")
    css = Path("static/styles.css").read_text(encoding="utf-8")

    assert 'class="icon-sprite"' in html
    assert all(f'id="{icon_id}"' in html for icon_id in ("i-chat", "i-search", "i-send", "i-paperclip", "i-settings"))
    ...
    assert ".composer textarea:focus-visible { outline: none; }" in css
    assert ".workspace { height: 100dvh; }" in css


def test_v56_locked_wal_checkpoint_is_deferred(monkeypatch):
    class LockedConnection:
        def execute(self, *_args, **_kwargs):
            raise main.sqlite3.OperationalError("database table is locked")

        def close(self):
            return None

    monkeypatch.setattr(main.sqlite3, "connect", lambda *_args, **_kwargs: LockedConnection())
...
def test_v65_unified_interface_contract():
    ...
    assert "Inteligencia personal confiable" in html
    assert "Planear una misión" in html
    assert 'aria-label="Actualizar panel"' in html
    assert "J.A.R.V.I.S. Reliable Intelligence v101" in Path("static/manifest.webmanifest").read_text(encoding="utf-8")
...
def test_v65_frontend_exposes_research_actions_and_multimodal_uploads():
    ...
    js = Path("static/app.js").read_text(encoding="utf-8")
    css = Path("static/styles.css").read_text(encoding="utf-8")
    assert "audio/*" in html and "Reliable Intelligence · v101" in html
